...
public class CombineDicomObject extends AbstractDicomObject {
	...
	public VR vrOf(int tag) {
		...
		return ds2.vrOf(resolveTagForDS2(tag));
	}

	public String nameOf(int tag) {
		if( ds1.contains(tag) ) return ds1.nameOf(tag);
		return ds2.nameOf(resolveTagForDS2(tag));
	}
	...
	public boolean contains(int tag) {
		return ds1.contains(tag) || ds2.contains(resolveTagForDS2(tag));
	}
	...
	/**
	 * Resolve the tag to use against the second internal DicomObject.
	 */
	private int resolveTagForDS2(int tag) {
		if(TagUtils.isPrivateDataElement(tag) && !TagUtils.isPrivateCreatorDataElement(tag)) {
			String creatorName = ds1.getPrivateCreator(tag);
			tag = ds2.resolveTag(tag, creatorName, false);
		}
		return tag;
	}
